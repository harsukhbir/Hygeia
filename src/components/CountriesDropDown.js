      ...
      <View style={[styles.taxesContainer, isIOS() && {zIndex: 1}]}>
        {
          <View style={styles.textInputContainer}>
            ...
            />
            {validateInput &&
              (isEmpty(searchCountryTerm) || countryErrorMessage !== '') && (
                <Text style={styles.error}>
                  {isEmpty(countryErrorMessage)
                    ? `${translate(
                        'signupScreen.countryPlaceholder',
                      )} is required`
                    : countryErrorMessage}
                </Text>
              )}
        ...
        {isShowDropDown === true && (
          <View style={styles.cardsContainer}>
            <View style={styles.cardsSection}>
              ...
              />
              {searchCountryTerm !== '' &&
                filterCountriesStates.length === 0 && (
                  <Text style={styles.notFoundContry}>
                    {translate('signupScreen.notFoundCountry')}
                  </Text>
                )}
            </View>
          </View>
